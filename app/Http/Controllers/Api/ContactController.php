...
class ContactController extends Controller
{
    ...
    public function save(Request $request): JsonResponse {
        ...
        try {
            ...
            DB::rollBack();
            logInfo($e->getMessage());
            return response()->json(['status' => false, 'message' => 'Duplicate phone number'], 400);
        }
        catch (\Exception $e) {
            DB::rollBack();
            logInfo(json_encode($e->getTrace()));
            return response()->json(['status' => false, 'message' => 'Please call system administrator'], 500);
        }
    }
    ...
    public function phoneNumbers($id): JsonResponse {
        try {
            $phoneNumbers = PhoneNumber::where('contact_id', $id)->with('contact')->get()->toArray();
